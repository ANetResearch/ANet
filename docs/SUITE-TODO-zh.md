...
| ~~D-27~~ | ~~部署靠手打,定时检查落后一个会话~~ | — | **已完成**。新增 `scripts/ship.sh`:八个二进制、五仓清单、经 cmax 中转、远端 sha256、**安装后回问每个服务跑的是哪个 commit**。漏掉 `anet-hub-admin` 那次正是"凭记忆拼出来的部署会漏掉当时不在眼前的东西"。另查出:cmax 上每小时跑的 prodtest 是 8/24 的 949 行版本(只到 9f 段),本地已 2235 行(到 9v)——**它一直报绿,而它验的与手工验的不是同一套**。`ship-prodtest.sh` 就是为这件事写的而无人调用,现已接进 `ship.sh` 并断言行数一致 |
| ~~D-28~~ | ~~付费闭环因赠额耗尽而永久静默跳过~~ | — | **已完成**。ink93 的注册赠额被历次测试花光,整套里最核心的那条路径(报价→授权→结算→干活→双链→余额)从此跳过。已充值,并改为余额低于 250 就提前报出充值命令 —— 一个只在预算已经没了的那一轮才出现的跳过,是在没人能补救的时候才说话 |
...
| ~~L-6~~ | ~~realworld 层从未跑过~~ | — | **已完成**。`test/realworld` 用 docker 起真 mosquitto、第三方 Modbus server、Home Assistant、OPC UA 与 Matter 设备,覆盖七个适配器 —— 而这一整层没跑过,**opcua 与 habridge 两个适配器因此一次都没被驱动**。现在 `scripts/realworld.sh` 一条命令跑完,dmax 上挂了每日 timer(`deploy/realworld.*`)。实测 7 项对着真服务通过、0 失败。查出三个缺陷:① `up.sh` 的 mediamtx 用 `--network host` 加固定端口,机器已占 `:8000` 时容器退出而 `up.sh` 照常打印 "L1 rig up" 并写入相机地址,测试于是以"摄像头不可达"失败 —— 真话但会让人去查适配器;② `run.sh` 只跑 `./adapters/...` 与 `./internal/...`,而 `matter/` 是仓根下自己的树 —— 容器每轮都起,适配器一次没验;③ 见 L-7 |
| ~~L-7~~ | ~~matter 发现遇到真实 mDNS 会死循环~~ | — | **已完成**。`matter/disc` 的 `record` 在 default 分支对所有段都调 `SkipAnswer`,而它在 authority/additional 段不前进解析器 —— 未处理的记录类型出现在那两段,`ingest` 就永远返回同一条,`Browse` 再不返回,调用方的 context 超时也救不了(goroutine 在空转不是在等)。安静网络不产生这种包,dmax 网段会:挂住它的包里是一台海康摄像头的 mDNS。另修配对窗口:matter.js 几分钟后关窗,测试把"窗口已关"报成了发现故障;判据取日志里**最后一次**窗口事件而非"是否出现过关闭"(重启后旧记录仍在,按出现与否判断会永远跳过 —— 只可能跳过的检查不是检查) |
| **L-1** | L2 真机测试 | 真机 + 凭据 | PTZ / 事件 / JPEG 抓拍。**海康 ISAPI 与大华 CGI 现在有模拟器了**(ANetMock,见 M-3),缺的变成"没有对真机测过"。dmax 网段上扫描到一台真 ONVIF 摄像头(210.45.70.239:9005),需要授权与凭据 |
