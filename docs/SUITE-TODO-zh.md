...
| **D-11** | 重投的聊天消息会在记录里重复 | 无 | 低。同一窗口的第三处,实测 2→3 条。修得对需要**消息级稳定标识** —— hub 邮箱行号只在 hub 路径上有,p2p 路径没有,所以这是 ANetCore 线格式的决定而不是能顺手打的补丁。副作用有限:重复行会被签进 transcript,但自洽可验,只是难看 |
